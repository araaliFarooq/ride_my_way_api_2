...
def get_requests_status(client_id):
    ...
    cursor.execute(
        "SELECT rideRequests.date, rideRequests.location, rideRequests.destination, rideOffers.driverName, rideOffers.carType, rideOffers.plateNumber, rideOffers.contact, rideRequests.status FROM rideRequests INNER JOIN rideOffers ON rideRequests.ride_id = rideOffers.ride_id WHERE rideRequests.client_id = '{}'" .format(client_id)
        )
    requests = cursor.fetchall()
    if not requests:
        return {"message": "No requests exist"}
